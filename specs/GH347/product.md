...
- Making the shared `_Powered by [Oz](https://oz.warp.dev)_` suffix configurable.
...
workflow_comments:
  ...
  triage-new-issues:
    follow_up_with_reporter: |
      ...
      ${questions_markdown}
```
This example shows how configured markdown can interpolate workflow-provided placeholders such as `${pr_url}`, `${spec_preview_markdown}`, `${next_steps_markdown}`, `${reporter_mention}`, and `${questions_markdown}` directly inside the YAML block scalar.
...
- helper-generated sections such as full `Next steps` blocks and spec-preview text
...
The shared `_Powered by [Oz](https://oz.warp.dev)_` suffix and the HTML metadata marker remain workflow-owned constants in the first release and are not configurable through `.github/oz/config.yml`.
...
- `${spec_preview_markdown}` for the generated spec-preview block
- `${next_steps_markdown}` for the full rendered next-steps block
