...
def _format_review_completion_message(
    workspace_root_or_event: str,
    event_or_recommended_reviewers: str | list[str],
    recommended_reviewers: list[str] | None = None,
) -> str:
    """Render the review completion message.

    The helper accepts both the current call shape
    ``(workspace_root, event, recommended_reviewers)`` and the legacy test
    shape ``(event, recommended_reviewers)``.
    """
    if recommended_reviewers is None:
        workspace_root = workspace()
        event = workspace_root_or_event
        reviewers = (
            event_or_recommended_reviewers
            if isinstance(event_or_recommended_reviewers, list)
            else []
        )
    else:
        workspace_root = workspace_root_or_event
        event = str(event_or_recommended_reviewers)
        reviewers = recommended_reviewers

    if event == "APPROVE":
        if reviewers:
            mentions = ", ".join(f"@{login}" for login in reviewers)
