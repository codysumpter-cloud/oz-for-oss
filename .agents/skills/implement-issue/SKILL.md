...
1. Read the issue details carefully. Review `spec_context.md` first when it exists, then review `issue_comments.txt` if it exists for any clarifications from organization members.
2. Inspect the repository to understand the current implementation before making changes.
3. Implement the requested behavior in the checked-out branch, keeping the changes scoped to the issue and aligned with any approved spec context.
...
6. Treat `issue_comments.txt`, `spec_context.md`, and `implementation_summary.md` as temporary workflow files only. Do not include them in the final diff.
